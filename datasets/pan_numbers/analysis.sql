...
select * from pan_numbers where pan_numbers is not null; 
...
select distinct * from pan_numbers;
...
-- New cleaned table:
create table pan_numbers_cleaned
as
...
select fn_check_adjacent_repetition('AXBCD')
...
select fn_check_sequence('AXCDE')
...
with cte as 
	(SELECT 
	    (SELECT COUNT(*) FROM pan_numbers) AS total_processed_records,
